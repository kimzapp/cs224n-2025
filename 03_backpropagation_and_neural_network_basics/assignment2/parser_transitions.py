...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            self.dependencies.append((self.stack[-1], second_word))
        elif transition == 'RA':
            first_word = self.stack.pop(-1)
            self.dependencies.append((self.stack[-1], first_word))
        else:
            raise NotImplementedError(f'Transition {transition} does not supported')
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for idx, partial_parse in enumerate(batch):
            partial_parse.parse_step(transition=transitions[idx])
